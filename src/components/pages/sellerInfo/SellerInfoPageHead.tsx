...
function getScoreText(curnScore: string | null, maxScore: string | null, siteId: number) {
...
function SellerInfoPageHead() {
  ...
  return (
    <PageHead
      ...
      description={`${getScoreText(curnScore, maxScore, site.id)} 총 ${commaNumber(
        reviewCount
      )}개의 후기를 받았고, ${commaNumber(totalCount)}개의 매물을 팔고 있어요.`}
      ogTitle={`판매자 ${name || id} 후기와 평점 보기 | 카멜`}
      ogDescription={`${getScoreText(curnScore, maxScore, site.id)} 총 ${commaNumber(
        reviewCount
      )}개의 후기를 받았고, ${commaNumber(totalCount)}개의 매물을 팔고 있어요.`}
